...
async function fromUrl(url, options = {}, signal) {
    return GeoTIFF.fromSource(await (0, remote_js_1.makeRemoteSource)(url, options), signal);
}
...
async function fromUrls(mainUrl, overviewUrls = [], options = {}, signal) {
    const mainFile = await GeoTIFF.fromSource(await (0, remote_js_1.makeRemoteSource)(mainUrl, options), signal);
    const overviewFiles = await Promise.all(overviewUrls.map((url) => (0, remote_js_1.makeRemoteSource)(url, options).then(source => GeoTIFF.fromSource(source))));
